...
> [!Note]
> In this fulfillment, **Amount** is a calculated item rather than a persistent entity in the data model.
>
> When the Sales Order has a payment plan, a separate Amount is calculated for each payment plan line. When there is no payment plan, a single Amount is calculated based on the total Sales Order amount, including VAT. Where applicable, the calculated amounts can be further broken down into invoiced and non-invoiced parts.
>
> The **Sales Order** remains the parent document of the generated Payment Orders, while **Amount** is used as the parent fulfillment item for calculating and tracking the fulfilled amounts.
