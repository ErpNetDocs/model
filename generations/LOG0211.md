...
2. Determine the relative part:  
   ...
   - Otherwise, the amount is adjusted using the formula:  
     ...
     ```  
**Important Note:**  
This generation supports both the use of *Additional Amounts* and *partial execution* with multiple `StoreTransactionLine` entries. However, using both simultaneously may result in rounding differences of up to **0.01** per `StoreTransactionLine`.  
