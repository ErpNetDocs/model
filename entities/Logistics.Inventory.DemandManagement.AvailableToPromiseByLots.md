...
| [FromDate](Logistics.Inventory.DemandManagement.AvailableToPromiseByLots.md#fromdate) | datetime __nullable__ | Тhe date from which the available to promise quantity is valid.`Filter(eq;ge;le)` |
...
Тhe date from which the available to promise quantity is valid.`Filter(eq;ge;le)`
