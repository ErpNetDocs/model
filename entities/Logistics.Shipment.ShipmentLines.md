...
| [Finished](Logistics.Shipment.ShipmentLines.md#finished) | boolean | 1 if this shipment should prohibit further shipments for the sales order line`Required` `Default(false)` `Filter(eq)` `ReadOnly` 
...
1 if this shipment should prohibit further shipments for the sales order line`Required` `Default(false)` `Filter(eq)` `ReadOnly`
