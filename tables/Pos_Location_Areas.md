...
Entity: [Crm.Pos.LocationAreas](~/entities/Crm.Pos.LocationAreas.md)
...
| - | - | --- |
|[Code](#code)|`nvarchar(16)` |Unique (within the location) short code, e.g., DINING, ZONE-A.|
|[Is_Active](#is_active)|`bit` |Indicates whether the POS location area  is currently active.|
|[Location_Id](#location_id)|`uniqueidentifier` |Parent shop, salon or facility.|
|[Name](#name)|`nvarchar(254)` `ML`|User-friendly name (e.g., "Main Dining Hall").|
|[Notes](#notes)|`nvarchar(max)` |Notes for the location area.|
|[Ord](#ord)|`int` |Sort order within the location. Controls display order in UI components.|
|[Pos_Location_Area_Id](#pos_location_area_id)|`uniqueidentifier` `PK`||
